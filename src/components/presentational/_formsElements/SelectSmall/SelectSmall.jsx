...
export class SelectSmallComponent extends React.Component {
	static $t;
	static i18n;
	constructor(props) {
		super(props);
	...
	updateScroll = () => {
		...
		const element = document.querySelector(`.select-small__list-item[tabindex="${index}"]`);
		if(element){
			element.focus();
		}
	}
	...
	setFocusByChar(char) {
    const elements = document.querySelectorAll(`.select-small__list-item`);
		KRUtils.each(elements, (key,val) => {
			let _string = val.innerText.toLowerCase();
			if(_string.substr(0,1) == char.toLowerCase()){
				this.setFocusedItem(this.props.items[val.getAttribute('tabindex')]);
				return false;
			}
		})
	}
	...
	getClassesForRoot() {
		return classnames(
			{
				"select-small": true,
				"select-small--showing": this.state.isShowing
			}
		);
	}
	renderList() {
	...
	renderValue() {
		...
		if(this.props.placeholder && (!value || value == "")) {
			return <div className="select-small__placeholder">{this.props.placeholder}</div>
		}
		return <div className="select-small__value">{this.getTittleOfValue()}</div>
	}
	render() {
		return (
			<div style={this.props.style || {}} className={this.getClassesForRoot()}>
				<span className="select-small__label">{this.props.label}</span>
				<div className="select-small__wrapper" tabIndex="0" onKeyDown={this.onPressKeyBoard} onClick={this.toggleShowingList}>
					{this.renderValue()}
					<span className="select-small__arrow"></span>
					<div ref={this.refScroll} className="select-small__list">
						{this.renderList()}
					</div>
				</div>
			</div>
...
SelectSmallComponent.propTypes = {
...
export const SelectSmall = withTranslation()(SelectSmallComponent)
